    ...
    class ImageSplit 
    {
        ...
        public List<DirectBitmap> LookForShapes()
        {
            ...
            while (pixelsToVisit.Count > 0 || helperQueue.Count > 0)
            {
                ...
                if (++numberOfVisited % 10000 == 0)
                    Trace.WriteLine(numberOfVisited);
            }
            Trace.WriteLine($"Number of figures: {figures.Count}");
            return figures;
        }
        ...
        private void AddPixelToFigure(Tuple<int, int> pixel)
        {
            ...
            figures[^1].SetPixel(pixel.Item1, pixel.Item2, Color.Red);
        }
    }
}
